...
export default function LaneWatches() {
  ...
  const [form, setForm] = useState({
    origin_state: '', dest_state: '', equipment_type: '', min_rate: '', min_rpm: '',
  });
  ...
  const handleCreate = () => {
    ...
    laneWatchesApi.create(payload)
      .then(w => {
        ...
        setShowForm(false);
        setForm({ origin_state: '', dest_state: '', equipment_type: '', min_rate: '', min_rpm: '' });
      })
  ...
  const laneLabel = (w) => {
    const from = w.origin_state || 'Any';
    const to   = w.dest_state   || 'Any';
    return `${from} → ${to}`;
  };
  ...
  return (
    <Box sx={{ maxWidth: 720, mx: 'auto' }}>
      ...
      <Typography variant="caption" color="text.disabled" sx={{ display: 'block', mt: 2, textAlign: 'center' }}>
        You can save up to 20 lane watches. Notifications appear in your bell icon.
      </Typography>
    </Box>
  );
}
