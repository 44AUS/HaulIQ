...
def upgrade():
    ...
    # 1. Create notificationtype enum — safe if already exists
    op.execute("""
        DO $$
        BEGIN
    ...
    # 2. notifications table — skip if already created by create_all
    ...
    # Index — safe to skip if already exists
    existing_indexes = [idx['name'] for idx in inspector.get_indexes('notifications')] if 'notifications' in existing_tables else []
    if 'ix_notifications_user_id_created' not in existing_indexes:
        op.create_index('ix_notifications_user_id_created', 'notifications', ['user_id', 'created_at'])

    # 3. lane_watches table — skip if already created by create_all
